...
5. When asked to **collect** new info from the net: write a new note under the right folder, add frontmatter (`status` / `updated` synced), append to `manifest.json` (include `status`, `updated`, `related`), list URLs under `sources/`, then run `npm run check` (validates + regenerates `INDEX.md`).
