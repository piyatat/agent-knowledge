...
- TypeScript: `npm install @openai/codex-sdk` (Node 18+, server-side). `new Codex()` → `startThread()` → `thread.run(prompt)`. Call `run` again on the same thread, or `resumeThread(threadId)` for a past one. Custom IDE/product clients that own auth, history, approvals, and streamed events should speak **app-server JSON-RPC** (`openai-codex-app-server`), not this SDK.
