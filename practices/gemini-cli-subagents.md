...
- Remote `kind: remote` and `experimental.adk.agentSessionSubagentEnabled` are separate; `/model` never overrides child `model`. Visual browser (`visualModel` + `analyze_screenshot`) needs API key or Vertex — not “Sign in with Google.”

## Sources

- [Subagents](https://geminicli.com/docs/core/subagents/) — accessed 2026-09-13
